import { units, snapshotMetadata, monthlyTotal, formatMoney } from "./data.js";
...
function render() {
  ...
  rows.innerHTML = list.map((unit, index) => `
    <article class="price-row" role="row">
      <div class="home-cell" role="cell">
        <span class="rank">${index + 1}</span>
        <div><strong>${unit.building} · ${unit.unit}</strong><small>${homeSpecs(unit)}</small></div>
      </div>
